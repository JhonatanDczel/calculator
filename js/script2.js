...
buttons.addEventListener('click', e => {
  ...
  if (btn.classList.contains('operations') && btn.id != '=') {
    ...
    firstNumber = parseInt(d2Value);
    d1Value = d2Value + btn.id;
    d2Value = '';
  }

  if (btn.id == '=') {
    secondNumber = parseInt(d2Value);
    d1Value = `${firstNumber} ${operator} ${secondNumber} =`;
    d2Value.textContent = operar(firstNumber, secondNumber, operator);
  }
  refreshDisplay();
});
